#!/bin/bash
INST_CLI_VER=aegis_00_35
INST_UPDATE_VER=update_00_35
...
install_aegis() {
    ...
    wget "${AEGIS_UPDATE_SITE}/$ARCH/${INST_UPDATE_VER}/aegis_update" -O "${AEGIS_INSTALL_DIR}/aegis_update/aegis_update" -T 5 -t 1
    if [ $? != 0 ]; then
        #echo "wget aegis_update error" 1>&2
        wget "${AEGIS_UPDATE_SITE2}/$ARCH/${INST_UPDATE_VER}/aegis_update" -O "${AEGIS_INSTALL_DIR}/aegis_update/aegis_update" -T 5 -t 1
        if [ $? != 0 ]; then
            echo "wget aegis_update error" 1>&2
            exit 1
        fi
    fi
    wget "${AEGIS_UPDATE_SITE}/$ARCH/${INST_UPDATE_VER}/agx_update.cfg" -O "${AEGIS_INSTALL_DIR}/aegis_update/agx_update.cfg" -T 5 -t 1
    if [ $? != 0 ]; then
        #echo "wget aegis_update conf error" 1>&2
        wget "${AEGIS_UPDATE_SITE2}/$ARCH/${INST_UPDATE_VER}/agx_update.cfg" -O "${AEGIS_INSTALL_DIR}/aegis_update/agx_update.cfg" -T 5 -t 1
        if [ $? != 0 ]; then
            echo "wget aegis_update conf error" 1>&2
            exit 1
        fi
...
install_service(){	
	if [ $LINUX_RELEASE = "GENTOO" ];then
		wget "${AEGIS_UPDATE_SITE}/aegis_gentoo" -O /etc/init.d/aegis -T 5 -t 1
		    if [ $? != 0 ]; then
		        #echo "wget aegis  error" 1>&2
		        wget "${AEGIS_UPDATE_SITE2}/aegis_gentoo" -O /etc/init.d/aegis -T 5 -t 1
		        if [ $? != 0 ]; then
		            echo "wget aegis error" 1>&2
		            exit 1
		        fi
		...
		wget "${AEGIS_UPDATE_SITE}/aegis" -O /etc/init.d/aegis -T 5 -t 1
		    if [ $? != 0 ]; then
		        #echo "wget aegis  error" 1>&2
		        wget "${AEGIS_UPDATE_SITE2}/aegis" -O /etc/init.d/aegis -T 5 -t 1
		        if [ $? != 0 ]; then
		            echo "wget aegis error" 1>&2
		            exit 1
		        fi
